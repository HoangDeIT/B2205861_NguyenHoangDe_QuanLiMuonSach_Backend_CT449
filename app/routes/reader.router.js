...
router.route("/")
    ...
    .patch(authenticateToken, authorizeRole("admin"), readers.updateById); // Cập nhật thông tin độc giả
router.route("/user")
    .get(authenticateToken, authorizeRole("user"), readers.findByUser)
    .patch(authenticateToken, authorizeRole("user"), readers.updatedByUser)
router.route("/user/change-password")
    .patch(authenticateToken, authorizeRole("user"), readers.changePassword)
router.route("/:id")
